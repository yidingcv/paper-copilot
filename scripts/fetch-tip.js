...
const VENUE_NAME = 'tip';
const DBLP_BASE_URL = 'https://dblp.org/db/journals/tip/tip';
...
function generatePaperId(title, authors, year) {
    ...
    return `${lastName}_${cleanTitle}_TIP_${year}_paper`;
}
...
async function startCrawl() {
    console.log(`--- 启动 TIP (Transactions on Image Processing) 抓取 ---`);

    for (const year of YEARS_TO_CRAWL) {
        // 计算卷号: 1992 是 Vol.1
        const volume = year - 1991;
        ...
        try {
            ...
            if ($('title').text().includes('Attention Required') || $('title').text().includes('Cloudflare')) {
                console.error(`❌ 被拦截：DBLP 触发了人机验证。请在浏览器中手动访问一次该 URL 并通过验证。`);
                continue;
            }
            ...
            entries.each((_, el) => {
                ...
                // 提取 IEEE 链接 (优先找 head 里的链接，那是直达 IEEE 的)
            ...
            if (yearPapers.length > 0) {
                ...
                console.log(`   ⚠️ 该年份未找到有效论文（可能 DBLP 尚未更新 Vol. ${volume}）`);
            }
